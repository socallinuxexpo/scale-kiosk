      ...
      perSystem =
            ...
            (nixpkgs.lib.nixosSystem {
              ...
              modules = [
                ...
                (
                  ...
                  {
                    services.timesyncd.enable = pkgs.lib.mkForce true;
                    # Use about:blank since core-conf.scale.lan is unreachable outside SCaLE LAN
                    services.kiosk-monitoring.url = "about:blank";
                    system.build.vmScript = pkgs.writeShellScriptBin "stateless-kiosk-monitoring-vm" ''
                      TMPDIR=$(mktemp -d)
                      function cleanup {
                        rm -rf "$TMPDIR"
                      }
        ...
        in
        {
          ...
          legacyPackages = rec {
            ...
            monitoring-vm = makeMonitoringVmScript { };
          };
        };
      flake =
        { ... }:
        {
          ...
          packages.aarch64-linux.pi-kiosk-sdImage =
            (self.nixosConfigurations.pi-kiosk.extendModules {
              modules = [
          ...
          packages.x86_64-linux.pi-kiosk-monitoring-sdImage =
            self.packages.aarch64-linux.pi-kiosk-monitoring-sdImage;
          packages.aarch64-linux.pi-kiosk-monitoring-sdImage =
            (self.nixosConfigurations.pi-kiosk-monitoring.extendModules {
              modules = [
                "${nixpkgs}/nixos/modules/installer/sd-card/sd-image-aarch64.nix"
                {
                  disabledModules = [ "profiles/base.nix" ];
          ...
          nixosConfigurations = {
            ...
            pi-kiosk-monitoring = nixpkgs.lib.nixosSystem {
              system = "aarch64-linux";
              specialArgs = { inherit inputs; };
              modules = [
                nixos-hardware.nixosModules.raspberry-pi-4
